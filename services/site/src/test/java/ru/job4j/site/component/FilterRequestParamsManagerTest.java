...
public class FilterRequestParamsManagerTest {
    ...
    void whenCreateRequestParamsWithNotAuthorFilter() {
        var filter = new FilterDTO(14, 0, 0, 3, 0);
        var expected = new FilterRequestParams(List.of(), 14, 0, 0, 0, true);
        assertThat(filterRequestParamsManager.getParams(filter)).isEqualTo(expected);
    }
    ...
    void whenCreateRequestParamsWithNotAgreedWisherFilter() {
        var filter = new FilterDTO(14, 0, 0, 4, 0);
        var expected = new FilterRequestParams(List.of(), 0, 0, 14, 0, true);
        assertThat(filterRequestParamsManager.getParams(filter)).isEqualTo(expected);
    }

    @Test
    void whenCreateRequestParamsWithWisherFilter() {
        var filter = new FilterDTO(14, 0, 0, 5, 0);
        var expected = new FilterRequestParams(List.of(), 0, 14, 0, 0, false);
        assertThat(filterRequestParamsManager.getParams(filter)).isEqualTo(expected);
    }
