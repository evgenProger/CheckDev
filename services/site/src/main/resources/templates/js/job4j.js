DOMAIN = document.domain === 'localhost' ? 'http://localhost' : 'http://136.243.21.15';
OAUTH_URL = DOMAIN + ':9900';
ARTICLE_URL = DOMAIN + ':9900';
ANNOUNCE_URL = DOMAIN + ':9900';
EXAM_URL = DOMAIN + ':9910';
NTF_URL = DOMAIN + ':9920';
INTERVIEW_URL = DOMAIN + ':9950';
FORUM_URL = DOMAIN + ':9960';
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.error) {
                    ...
                    $('#forgot_success').html("Новый пароль был отправлен Вам на почту.");
                    $('#forgot_success').show();
                }
            },
...
function login(selector) {
    ...
    if (formIsValid(selector)) {
        ...
        $.ajax({
            ...
            crossDomain: true,
            dataType: "json",
            data : {"scope" : "any", "grant_type" : "password", "username" : email, "password" : $('#password').val()},
            beforeSend : function( xhr ) {
            ...
            error : function (error) {
                ...
                if (result.error) {
                    alert(result.error == 'invalid_grant' ? "Логин или пароль не совпадают." : result.error_description);
                    $(selector + " :input").prop("disabled", false);
                } else {
                    closeDialog(result);
...
function order(selector, type) {
    $(selector).validator('validate');
    if (formIsValid(selector)) {
        $(selector + " :input").prop("disabled", true);
        $.ajax({
            url: OAUTH_URL + '/order/save',
            type: 'POST',
            crossDomain: true,
            dataType: "json",
            data: JSON.stringify({
            ...
            success: function (data) {
                if (data.error) {
                    ...
                    $('#order_congratulate').show();
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                if (errorThrown) {
                    $('#order_error').html(errorThrown);
                }
                $('#order_error').show();
                $(selector + " :input").prop("disabled", false);
            }
...
function doAction($http, admin, user, error, roleAdmin, roleModerator) {
    doGet($http, OAUTH_URL + '/person/current',
        function (data) {
            var isAdmin = $.grep(data.roles, function(role) {
                return (roleAdmin && roleAdmin === role.value) || (roleModerator && roleModerator === role.value);
            });
            if (isAdmin.length > 0) {
                admin();
            } else {
