...
public interface PersonRepository extends CrudRepository<Person, Integer> {
    ...
    @Query("select new ru.job4j.auth.domain.Person(p.key, p.experience, p.salary, p.aboutShort, p.username, p.location, p.photo.id) from person as p left JOIN p.photo where p.show is ?1 order by p.updated desc")
